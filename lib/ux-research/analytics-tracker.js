...
class AnalyticsTracker extends BaseManager {
  constructor(options = {}) {
  ...
  saveData() {
    ...
    const filePath = path.join(dataDir, 'analytics-data.json');
    try {
      fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    } catch (error) {
      this.logError(error, { action: 'saveData', filePath });
      throw error;
    }
