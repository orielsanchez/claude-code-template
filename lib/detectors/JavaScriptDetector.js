...
class JavaScriptDetector extends AbstractDetector {
  ...
  detectFrameworks(projectPath, result) {
    ...
    const packageData = parser.parse(packageJsonPath);
    if (!packageData) {
      this.dependencies = {};
    ...
    this.dependencies = { ...packageData.dependencies, ...packageData.devDependencies };
    ...
    result.testFrameworks.push(...frameworkResult.testFrameworks);
    this.primaryFramework = frameworkResult.primary;
