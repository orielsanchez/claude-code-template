...
class ConfigurationManager {
  constructor(options = {}) {
    ...
    this.logger = createContextualLogger('ConfigurationManager');
  ...
  async loadConfiguration(cliOptions = {}) {
    ...
    try {
      ...
      this.logger.warn('Failed to load main config file', {
    ...
    if (!validation.valid) {
      this.logger.warn('Configuration validation errors', {
        operation: 'loadConfiguration',
    ...
    try {
      ...
      for (const [name, plugin] of this.plugins) {
        try {
          ...
          this.logger.warn('Failed to save plugin config', {
  ...
  async migrateConfiguration() {
    ...
    try {
      ...
      for (const [name, plugin] of this.plugins) {
        if (plugin.migrate) {
          try {
            ...
            this.logger.warn('Failed to migrate plugin', {
              operation: 'migrateConfiguration',
              pluginName: name,
              errorMessage: error.message
            });
          }
      ...
      this.logger.warn('Failed to migrate configuration', {
        operation: 'migrateConfiguration',
        errorMessage: error.message
      });
      return this.defaults;
