...
class ThemeManager {
  constructor(sharedData = {}) {
    ...
    this.logger = createContextualLogger('ThemeManager');
    ...
    if (sharedData.configManager) {
      ...
      this.dataDir = path.join(this.configManager.configDir, 'theme');
  ...
  async loadTheme(themeName) {
    ...
    try {
      ...
      this.logger.warn('Failed to load custom theme', {
        operation: 'loadCustomTheme',
        themeName,
  ...
  getCurrentTheme() {
    if (this.configManager) {
      const pluginConfig = this.configManager.getPluginConfig('theme');
      return pluginConfig.currentTheme || this.currentTheme;
    }
  ...
  async switchTheme(themeName) {
    ...
    if (this.configManager) {
      this.configManager.updatePluginConfig('theme', { currentTheme: themeName });
      
      // Emit theme change event
      this.configManager.emit('theme:changed', {
        plugin: 'theme',
        action: 'theme-changed',
        data: { newTheme: themeName, oldTheme }
      });
    } else {
  ...
  /**
   * Get recommended theme based on accessibility settings
   * @returns {Promise<string>} Recommended theme name
   */
  async getRecommendedTheme() {
    if (this.configManager) {
      const sharedData = this.configManager.getSharedData();
      if (sharedData.accessibility) {
        if (sharedData.accessibility.highContrast || sharedData.accessibility.screenReader) {
