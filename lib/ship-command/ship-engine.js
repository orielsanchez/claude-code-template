...
class ShipEngine extends BaseManager {
  /**
   ...
   */
  constructor(options = {}) {
    // Initialize BaseManager with appropriate options
    const baseOptions = {
      userId: options.userId || 'ship-user',
      configManager: options.configManager,
      pluginName: 'ship-command'
    };
    super(baseOptions);
    
    // Keep workingDir separate from dataDir (ShipEngine specific)
  ...
  async _createErrorResult(type, message) {
    ...
    // Log error using BaseManager's error logging
    this.logError(new Error(message), { type, action: 'ship' });
