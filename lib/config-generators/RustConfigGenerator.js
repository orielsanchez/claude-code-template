...
class RustConfigGenerator extends BaseConfigGenerator {
  ...
  generate() {
    ...
    // Framework-specific commands and configurations
...
`);
  ...
  generateFrameworkConfig() {
    ...
    // Tokio async runtime
    if (this.detected.frameworks?.includes('tokio')) {
      this.addClaudeMdAdditions('\n## Tokio Async Runtime\n\n- Use async/await patterns\n- Implement proper async error handling\n- Follow Tokio best practices\n');
    }

    // Axum web framework
    if (this.detected.frameworks?.includes('axum') || this.detected.primary === 'axum') {
      this.addClaudeMdAdditions('\n## Axum Framework\n\n- Use tower middleware for cross-cutting concerns\n- Implement proper error handling with axum::Error\n- Follow async/await patterns\n');
    }

    // Actix-web framework
    if (this.detected.frameworks?.includes('actix-web') || this.detected.primary === 'actix-web') {
      this.addClaudeMdAdditions('\n## Actix-web Framework\n\n- Use actix-web middleware\n- Implement proper async handlers\n- Follow Actix patterns\n');
    }

    // Diesel ORM
    if (this.detected.frameworks?.includes('diesel')) {
      this.addCommands({
        migration: 'diesel migration run',
        setup: 'diesel setup'
      });
      this.addClaudeMdAdditions('\n## Diesel ORM\n\n- Use migrations for database schema\n- Follow Diesel query patterns\n');
    }

    // Criterion benchmarking
    if (this.detected.frameworks?.includes('criterion') || this.detected.testFrameworks?.includes('criterion')) {
      this.addClaudeMdAdditions('\n## Criterion Benchmarking\n\n- Write performance benchmarks\n- Use criterion for micro-benchmarks\n');
    }

    // Cross-compilation
    if (this.detected.tools?.includes('cross') || this.detected.frameworks?.includes('cross-compilation') || this.detected.hasCrossCompilation) {
      this.addClaudeMdAdditions('\n## cross-compilation\n\n- Use cross for cross-platform builds\n- Test on target platforms\n');
    }

    // Workspace configuration
    if (this.detected.tools?.includes('workspace') || this.detected.frameworks?.includes('workspace') || this.detected.isWorkspace) {
      this.addClaudeMdAdditions('\n## Workspace Configuration\n\n- Organize code into workspace members\n- Share dependencies across workspace\n');
    }
  }
}
