...
class SwiftConfigGenerator extends BaseConfigGenerator {
  ...
  generate() {
    ...
    // Add Swift 6 specific hooks if modern features detected
    if (this.detected.frameworks?.includes('swift-6-mode') || 
        this.detected.frameworks?.includes('strict-concurrency')) {
      hooks.push('swift build -Xswiftc -strict-concurrency=complete');
    }

    // Add testing hook - coverage enabled for modern projects
    if (this.detected.frameworks?.includes('swift-testing') || 
        this.detected.frameworks?.includes('swift-6-mode')) {
      hooks.push('swift test --enable-code-coverage');
    }
    
    // Always include basic swift test for compatibility
    hooks.push('swift test');

    hooks.push('swift package resolve');
    hooks.push('swift package show-dependencies || echo "Not a Swift package, skipping dependency check"');
    ...
    // Add Swift 6 modern testing commands
    if (this.detected.testFrameworks?.includes('swift-testing')) {
      commands['test-swift-testing'] = 'swift test';
    }

    // Add documentation generation commands for modern Swift projects
    if (this.detected.frameworks?.includes('swift-docc') || 
        this.detected.frameworks?.includes('spm-plugins') ||
        this.detected.frameworks?.includes('swift-6-mode') ||
        this.detected.frameworks?.includes('swift-testing')) {
      commands['generate-docc'] = 'swift package generate-documentation';
      commands['preview-docs'] = 'swift package preview-documentation';
    }
    ...
    // Add Swift 6 specific CLAUDE.md content
    if (this.detected.frameworks?.includes('swift-6-mode') || 
        this.detected.frameworks?.includes('swift-concurrency') ||
        this.detected.frameworks?.includes('swiftui')) {
      
      const swiftAdditions = [];
      
      if (this.detected.frameworks?.includes('swift-6-mode') || 
          this.detected.frameworks?.includes('swift-concurrency')) {
        swiftAdditions.push('Swift 6');
        swiftAdditions.push('Concurrency');
      }
      
      if (this.detected.frameworks?.includes('swiftui')) {
        swiftAdditions.push('SwiftUI');
      }
      
      if (swiftAdditions.length > 0) {
        this.config.claudeMdAdditions = swiftAdditions;
      }
    }
